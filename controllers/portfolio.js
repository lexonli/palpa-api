 ...
 * @param isOwner {boolean} - is the person using this endpoint the owner of the portfolio
      ...
      q.Let({},
        {
          ...
          contactMessage: q.Select(["data", "contactMessage"], q.Var("userDoc")),
          socialLinks: q.Select(["data", "socialLinks"], q.Var("userDoc")),
          //projects of the user
          projects: q.Let(
            //setup variables, processedProjects is an array of all projects of the user
            {
                ...
                q.Lambda("project",
                  q.Let(
                    ...
                    { name:q.Select(["data", "name"], q.Var("projectDoc")),
                      description:q.Select(["data", "description"], q.Var("projectDoc")),
                      image:q.Select(["data", "image"], q.Var("projectDoc")),
                      isPublished:q.Select(["data", "isPublished"], q.Var("projectDoc"))
                    }
                  )
                )
              )
            },
            //return value, if the person using this is an owner, we give them unpublished projects as well
            //otherwise, we filter them out
            q.If(q.Var("isOwner"),
              q.Var("processedProjects"),
              q.Filter(
                q.Var("processedProjects"),
                q.Lambda("processedProject",q.Select(["isPublished"], q.Var("processedProject")))
              ),
            )
          ),
          ...
          experiences: q.Map(
           q.Select(["data", "experiences"], q.Var("userDoc")),
            q.Lambda("experience",
              q.Let(
                //setup variables, experienceDoc is a ref to a single experience of the user
                { experienceDoc: q.Get(q.Var("experience")) },
                { title:q.Select(["data", "title"], q.Var("experienceDoc")),
                  employmentType:q.Select(["data", "employmentType"], q.Var("experienceDoc")),
                  //convert dates to epoch times in the form of (eg. 1230728400)
                  startDate: q.TimeDiff(q.Epoch(0, 'second'), q.Select(["data", "startDate"], q.Var("experienceDoc")), 'second'),
                  //endDate might be null, so we only convert to epoch time if it is not null, otherwise just return null
                  endDate: q.Let(
                    { end: q.Select(["data", "endDate"], q.Var("experienceDoc"), null) },
                    q.If(q.Equals(q.Var("end"), null), null, q.TimeDiff(q.Epoch(0, "second"), q.Var("end"), "second"))
                  ),
                  company:q.Select(["data"], q.Get(q.Select(["data", "company"], q.Var("experienceDoc"))))
                }
              )
            )
          )
